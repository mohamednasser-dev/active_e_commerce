          ...
          <form class="p-4" action="{{ route('flash_deals.update', $flash_deal->id) }}" method="POST">
            @csrf
              ...
              <input type="hidden" name="lang" value="{{ $lang }}">
                ...
                @php
                  $start_date = date('Y-m-d', $flash_deal->start_date);
                  $end_date = date('Y-m-d', $flash_deal->end_date);
                @endphp

                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="start_date">{{translate('Date')}}</label>
                    <div class="col-sm-9">
                      <input type="text" class="form-control aiz-date-range" value="{{ $start_date.' to '.$end_date }}" name="date_range" placeholder="Select Date" data-past-disable="true" data-separator=" to ">
                    </div>
                </div>
                <div class="form-group row">
