...
<div class="card">
    ...
    <div class="card-body">
        <table class="table aiz-table mb-0">
            ...
            <tr>
                ...
                <th>{{translate('shiping type')}}</th>
              
                <!--th>{{translate('koshk shiping')}}</th>
                <th>{{translate('have shiping')}}</th -->
            ...
            @foreach($sellers as $key => $seller)
                @if($seller->user != null)
                    <tr>
                        ...
                        <td>{{$seller->user->shipingtype}}</td>
                         
                        <!--td>
                          

                            <label class="aiz-switch aiz-switch-success mb-0">
          <input onchange="koshkshipingstat(this)" value="{{ $seller->id }}" type="checkbox" <?php if($seller->user->koshkshiping == 1) echo "checked";?> >
 ...
 <label class="aiz-switch aiz-switch-success mb-0">
          <input onchange="haveshipingstat(this)" value="{{ $seller->id }}" type="checkbox" <?php if($seller->user->haveshiping == 1) echo "checked";?> >
                                <span class="slider round"></span>
                            </label>
                        </td -->
                        <td>
    ...
    <script type="text/javascript">
        ...
        function update_approved(el){
         ...
         function koshkshipingstat(el){
            ...
            $.post('{{ route('sellers.koshkshipingapproved') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('koshk shiping stat updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
         ...
         function haveshipingstat(el){
            ...
            $.post('{{ route('sellers.haveshipingapproved') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('have shiping  stat updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
